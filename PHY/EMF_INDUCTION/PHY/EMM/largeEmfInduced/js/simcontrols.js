...
$(document).ready(function() {
    ...
    $("#checkCir").click(function() {
        ...
        if ($(this).is(":checked")) {
            ...
            $("#bulbLeft,#bulbRight,#inductLeft,#inductRight,#inductBulbLeft,#inductBulbRight,#switchLeft,#switchRight,#batPos,#batNeg").css({
                "pointer-events": "none"
            })
        } else {
            $("#circuitDiagram").hide();
...
function switchOff() {
    ...
    $("#key").draggable({
        ...
        drag: function() {
            ...
            $("#bulbLow,#bulbBright,#bulbMedium").css({
                "display": "none"
            });
            $("#keyDropArea").css({
                'display': "block"
    ...
    $("#keyDropArea").droppable({
        ...
        drop: function() {
            ...
            $("#resultData").hide();
            $("#keyOutArea,#keyOutArea1,#keyOutArea2,#keyOutArea3").css({
                'display': "block"
            })
        }
    })
...
function ImageInitLoad(ANIMATIONImageArray, ANIMATIONDivName, ANIMATIONImageIdName, ANIMATIONImageClassName) {
    for (var ANIMATIONForloop = 0; ANIMATIONForloop < ANIMATIONImageArray.length - 1; ANIMATIONForloop++) {
        var $tempx2 = $("<img src='" + ANIMATIONImageArray[ANIMATIONForloop] + "'/>").attr({
            id: ANIMATIONImageIdName + ANIMATIONForloop,
            class: ANIMATIONImageClassName,
            display: "none"
        });
...
function ImageAnimationOneTime(ANIMATIONImageClassName, ANIMATIONImageIdName, ANIMATIONImageArray, ANIMATIONSpeed, ANIMATIONTimes) {
    ...
    var CLEARImageAnimation = setInterval(function() {
        $('#' + ANIMATIONImageIdName + (GlobalVariable - 1)).css({
            'display': 'none'
        });
        $('#' + ANIMATIONImageIdName + GlobalVariable).css({
            'display': 'block'
        });
        GlobalVariable++;
        if (GlobalVariable == ANIMATIONImageArray.length - 1) {
            $('.' + ANIMATIONImageClassName).css({
                'display': 'none'
            });
            $('#' + ANIMATIONImageIdName + (0)).css({
                'display': 'block'
            });
